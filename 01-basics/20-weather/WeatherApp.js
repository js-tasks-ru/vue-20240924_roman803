...
export default defineComponent({
  ...
  setup() {
    ...
    function getIcon(data) {
      return WeatherConditionIcons[data.current.weather.id]
    }

    function determineNightOrDay(data) {
      const currentTime = data.current.dt
      if (
        currentTime < data.current.sunset
        && currentTime > data.current.sunrise
      ) {
        return false
      }
    ...
    <div>
      ...
      <ul class="weather-list unstyled-list">
        <li 
          ...
          class="weather-card"
          :class="{ 'weather-card--night': determineNightOrDay(item) }"
        >
          ...
          <div class="weather-conditions">
            <div class="weather-conditions__icon" :title="item.current.weather.description">{{ getIcon(item) }}</div>
            <div class="weather-conditions__temp">{{ (item.current.temp -  273.15).toFixed(1) + ' °C'}}</div>
          </div>
          <div class="weather-details">
            <div class="weather-details__item">
              <div class="weather-details__item-label">Давление, мм рт. ст.</div>
              <div class="weather-details__item-value">{{ (item.current.pressure * .75).toFixed(0) }}</div>
            </div>
            <div class="weather-details__item">
